...
#Specifying the url for FDOT Plaaned Advertisements
url <- 'http://www2.dot.state.fl.us/procurement/ProfessionalServices/advertise/advplan.shtml'
...
if(!file.exists("fdot_ads_Planned.xlsx")) {
  write.xlsx(tAds, "fdot_ads_Planned.xlsx")
  write.xlsx(tAds, paste0("./Archive_Planned/fdot_ads_Planned",format(Sys.Date(), "_%b_%d_%Y"),".xlsx"), row.names=FALSE)
  
} else if (nrow(tAds) == nrow(read.xlsx("fdot_ads_Planned.xlsx")) && ncol(tAds) == ncol(read.xlsx("fdot_ads_Planned.xlsx"))){
  if (tAds == read.xlsx("fdot_ads_Planned.xlsx")) {
  winDialog(type = "ok", "No new advertisements!")
  write.xlsx(tAds, paste0("./Archive_Planned/fdot_ads_Planned",format(Sys.Date(), "_%b_%d_%Y"),".xlsx"), row.names=FALSE)
  }
} else {
  newAds <- anti_join(tAds, read.xlsx("fdot_ads_Planned.xlsx"), by = c( "District", "Attn.", "Contract", "Phone", "Advertisement Date" = "Advertisement.Date", "Final Selection Meeting Date" = "Final.Selection.Meeting.Date"))
 if(nrow(newAds) == 0) {
   winDialog(type = "ok", "No new advertisements!")
   write.xlsx(tAds, paste0("./Archive_Planned/fdot_ads_Planned",format(Sys.Date(), "_%b_%d_%Y"),".xlsx"), row.names=FALSE)
   write.xlsx(tAds, "fdot_ads_Planned.xlsx", row.names=FALSE)
   
 } else {
  winDialog(type = "ok", paste0("There are ", nrow(newAds), " new ads that is saved in fdot_ads_Planned_OnlyNewAds.xlsx"))
  write.xlsx(tAds, "fdot_ads_Planned.xlsx", row.names=FALSE)
  write.xlsx( newAds,  "fdot_ads_Planned_OnlyNewAds.xlsx", row.names=FALSE)
  write.xlsx(tAds, paste0("./Archive_Planned/fdot_ads_Planned",format(Sys.Date(), "_%b_%d_%Y"),".xlsx"), row.names=FALSE)
 }
}
}
